...
export class FirestoreService {
  ...
  col<T>(ref: CollectionPeriodicate<T>, queryFn?: QueryFn<firebase.default.firestore.DocumentData> | undefined): AngularFirestoreCollection<T> {
  ...
  col$<T>(ref: CollectionPeriodicate<T>, queryFn?: QueryFn<firebase.default.firestore.DocumentData> | undefined): Observable<T[]> {
    return this.col(ref, queryFn).snapshotChanges().pipe(map(docs => {
      return docs.map(a => a.payload.doc.data()) as T[]
    }))
  }

  colWithId$<T>(ref: CollectionPeriodicate<T>, queryFn?: QueryFn<firebase.default.firestore.DocumentData> | undefined): Observable<T[]> {
  ...
  colWithIdNum$<T>(ref: CollectionPeriodicate<T>, queryFn?: QueryFn<firebase.default.firestore.DocumentData> | undefined): Observable<T[]> {
    return this.col(ref, queryFn).snapshotChanges().pipe(map(actions => {
      return actions.map(a => {
        const data = a.payload.doc.data();
        const id = parseInt(a.payload.doc.id);
        return {id, ...data}
      });
    }));
  }
